...
import { getProduct, editProduct } from '../services/db';
...
export const EditProduct: React.FC = () => {
  ...
  // Custom Cloudinary settings
  const [cloudName, setCloudName] = useState('dfziz6t0o'); // User config
  const [uploadPreset, setUploadPreset] = useState('clacks'); // Spec config
    ...
    const loadProductData = async () => {
      ...
      try {
        ...
        if (product) {
          ...
          setCategory(product.category || 'Sneakers');
          setDescription(product.description || '');
          setStock(product.stock || 0);
          setSelectedSizes(product.sizes || []);
          setImageUrl(product.image || '');
        } else {
          setErrorStatus(`Product with ID "${id}" could not be located in database collections.`);
        }
      } catch (err: any) {
        console.error(err);
        setErrorStatus("Error fetching product. Verify database connections.");
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleUploadToCloudinary = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Cloudinary responded with error status: ${response.status}`);
      }
      ...
      console.error(err);
      setErrorStatus(`Cloudinary Upload Failed. Check your settings. Fell back to standard mock URL.`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAdmin) {
      setErrorStatus("Unauthorized Action: Only the verified owner biestjhon78@gmail.com can modify product catalogs.");
      return;
    }
    ...
    if (selectedSizes.length === 0) {
      setErrorStatus("Please select at least one available size.");
      return;
    }
    if (!imageUrl) {
      setErrorStatus("Please upload an image or provide an image link.");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setErrorStatus(err.message || "An error occurred writing changes to Firestore.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div className="h-96 flex flex-col justify-center items-center">
        ...
        <p className="text-xs text-[#a8a29e] tracking-widest uppercase font-light">Retrieving product specs...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-gold-500">
          Edit State: Amending footwear {id?.substring(0, 8)}...
        </span>
      </div>

      <div className="bg-[#0A0A0A] border border-white/5 rounded-2xl p-8 shadow-2xl">
        <div className="border-b border-white/5 pb-5 mb-8 flex justify-between items-start">
          <div>
            <h3 className="font-display font-light text-2xl tracking-wide text-white">Amend Footwear Entry</h3>
            <p className="text-[#737373] text-xs font-light mt-1.5">Modify parameters, restock supplies, and amend sizes.</p>
          </div>
          {imageUrl && (
            <div className="w-16 h-16 bg-[#050505] border border-white/5 rounded overflow-hidden">
              <img src={imageUrl} alt="current" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
            </div>
          )}
        </div>
        ...
        {!isAdmin && (
          <div className="mb-6 p-4 rounded bg-yellow-600/5 border border-yellow-500/20 text-yellow-500 text-xs flex items-center gap-2">
            ...
            <span>You are browsing in Guest/Viewer mode. Amending products in Firestore will be blocked by security rules.</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] uppercase tracking-wider text-[#737373] font-light">Price ($)</label>
                <input 
                  type="number" 
                  min="0"
                  value={price || ''}
          ...
          <div className="border border-white/5 bg-black/40 rounded-2xl p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input 
                  ...
                  className="w-full bg-black border border-white/5 rounded p-2 text-[11px] font-mono text-white focus:border-[#BFA181]/60 outline-none"
                />
              </div>
            ...
            {/* Image pickers trigger */}
            <div className="pt-2 flex flex-col sm:flex-row items-center gap-4">
              <div className="flex-1 w-full space-y-1.5">
                <label className="text-[10px] uppercase tracking-wide text-[#737373] block">1. (Optional) Choose New Image</label>
            ...
            <div className="space-y-1.5 pt-2">
              <label className="text-[10px] uppercase tracking-wide text-[#737373]">Product Image URL</label>
              <input 
                type="text" 
                value={imageUrl}
                onChange={e => {
